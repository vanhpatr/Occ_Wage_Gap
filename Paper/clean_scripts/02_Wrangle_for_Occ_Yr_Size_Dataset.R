...
## Purpose: Using ACS data, produce year by year summary of occupation size and average
## wages. Decide the sample of occupations. 
...
dta.files <- list.files("~/Desktop/Occ_Wage_Gap/Paper/data_mod/")
dta.files.yr<- dta.files[str_detect(dta.files, "-") == FALSE & str_detect(dta.files, "JUL2026_acs") == TRUE ]
# Read clean occupation-level data
occ <- read_rds("~/Desktop/Occ_Wage_Gap/Paper/data_raw/clean_occupations.rds")
...
for(i in 1:length(dta.files.yr)){
  dta <- read_rds(paste0("~/Desktop/Occ_Wage_Gap/Paper/data_mod/"
                         , dta.files.yr[i])) %>%
...
dta <- read_rds("~/Desktop/Occ_Wage_Gap/Paper/data_mod/JUL2026_acs_2001_estimates.rds")
...
final_sample  <- final_sample%>%
  mutate(OCC2010 = as.character(zap_labels(OCC2010)))%>%
  left_join( occ_title_cw,by = "OCC2010")
 
overall_data <- overall_data %>%
  mutate(sh.mig = n_immigrant/(n_immigrant+n_native))

write_rds(final_sample, "~/Desktop/Occ_Wage_Gap/Paper/data_mod/20260703_occ_summary_nonparametric.rds")
write_rds(overall_data, "~/Desktop/Occ_Wage_Gap/Paper/data_mod/20260703_year_summary_nonparametric.rds")
write_rds(occ_title_cw, "~/Desktop/Occ_Wage_Gap/Paper/data_mod/occ_title_crosswalk.rds")
